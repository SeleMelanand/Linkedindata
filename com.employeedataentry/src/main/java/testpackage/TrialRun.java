...
public class TrialRun {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		driver.findElementByXPath("//artdeco-typeahead-input[@class='ember-view']/input")
				.sendKeys("First American Financial Corporation", Keys.ENTER);
		...
		String personlinked = driver
				.findElementByXPath("//div[contains(@class,'pv-top-card-section__information')]/div/h1").getText();
		String persondesignation = driver.findElementByXPath("//h2[contains(@class,'pv-top-card-section__headline')]")
				.getText();
		...
		System.out.println("Total number of companies worked - " + abc.size());
		int counter = 1;
		for (int i = 0; i < 10; i++) {
			if (i < abc.size() - 1) {
				for (WebElement a1 : abc) {
					try {
						Experiencedetails = driver
								.findElementByXPath("(//div[@class='pv-entity__summary-info'])[" + counter + "]/h3")
								.getText();
					} catch (Exception e) {
					...
					try {
						companyname = driver
								.findElementByXPath(
										"(//div[@class='pv-entity__summary-info'])[" + counter + "]/h4[1]/span[2]")
								.getText();
					} catch (Exception e) {
						System.out.println("Experience details doesnot exists- " + personlinked);
					}
					try {
						period = driver
								.findElementByXPath(
										"(//div[@class='pv-entity__summary-info'])[" + counter + "]/h4[2]/span[2]")
								.getText();
					} catch (Exception e) {
						System.out.println("Period doesnot exists- " + personlinked);
					}
					try {
						employmentduration = driver
								.findElementByXPath(
										"(//div[@class='pv-entity__summary-info'])[" + counter + "]/h4[3]/span[2]")
								.getText();
					} catch (Exception e) {
						System.out.println("employmentduration doesnot exists- " + personlinked);
					}
					try {
						employmentlocation = driver
								.findElementByXPath(
										"(//div[@class='pv-entity__summary-info'])[" + counter + "]/h4[4]/span[2]")
								.getText();
					} catch (Exception e) {
				...
				int tempcount = employmentlist.size();
				System.out.println(tempcount);
				employmentlist.add(i, null);
				
			
		}

		for (int k = 0; k <10; k++) {
			System.out.println(employmentlist.get(k));
		}
	}
