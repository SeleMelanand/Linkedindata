...
public class UserDetails {
	...
	@Test
	public void employeedetails() throws AWTException, InterruptedException, IOException {
		...
		driver.findElementByXPath("//artdeco-typeahead-input[@class='ember-view']/input")
				.sendKeys("First American Financial Corporation");
		driver.findElementByXPath("//artdeco-typeahead-input[@class='ember-view']/input").sendKeys(Keys.ENTER);
		driver.findElementByXPath("//button[@data-vertical='PEOPLE']").click();
		Thread.sleep(2000);

		
		WebElement element = driver.findElementByXPath("//div[@class='next-text']");
		// Pagedown to be made to see the next button.
		...
		int titlecount = titlesearch.length;
		// Loop for running all the different title for the specific company
		for (int b = 0; b < 1; b++) {
			driver.findElementById("advanced-search-title").sendKeys(titlesearch[b], Keys.ENTER);
			Thread.sleep(6000);

			// To get the total number of employees linked to the company which is searched
			...
			for (int i = 1; i <= (iteratequotient + 1); i++) {
				...
				for (int j = 0; j < count; j++) {
					...
					WebElement elementdown = driver.findElementByXPath("(//span[contains(@class,'actor-name')])[7]");
					...
					Thread.sleep(3000);
					String name = driver.findElementByXPath("(//span[contains(@class,'actor-name')])[" + n + "]")
							.getText();
					...
					if (name.equalsIgnoreCase("LinkedIn Member")) {
						...
						String personlinked = driver
								.findElementByXPath("//div[contains(@class,'pv-top-card-section__information')]/div/h1")
								.getText();
						String persondesignation = driver
								.findElementByXPath("//h2[contains(@class,'pv-top-card-section__headline')]").getText();

						String employer = driver
								.findElementByXPath("//h3[contains(@class,'pv-top-card-section__company')]").getText();
						...
						for (int empcounter = 1; empcounter <= 3; empcounter++) {
							...
							try {
								Experiencedetails = driver
										.findElementByXPath(
												"(//div[@class='pv-entity__summary-info'])[" + empcounter + "]/h3")
										.getText();
							...
							try {
								companyname = driver.findElementByXPath(
										"(//div[@class='pv-entity__summary-info'])[" + empcounter + "]/h4[1]/span[2]")
										.getText();
							} catch (Exception e) {
						...
						List<WebElement> education = driver
								.findElementsByXPath("//h3[contains(@class,'pv-entity__school-name')]");
						...
						for (int m = 1; m <= educationcount; m++) {
							...
							try {
								educationperiod = driver
										.findElementByXPath(
												"(//p[contains(@class,'pv-entity__dates')])[" + m + "]/span[2]")
										.getText();
							} catch (Exception e) {
							...
							String educationdetails = educationinstitute + ",  " + degreedetails + ",  "
									+ educationperiod;
		...
		System.out.println("All record saved Successfully !!!!!!!!");
